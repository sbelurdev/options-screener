...
def write_reports(candidates: List[Dict[str, Any]], config: Dict[str, Any], disclaimer: str) -> Tuple[str, str]:
    ...
    html_parts.append(
        ...
        "th,td{border:1px solid #d0d7de;padding:6px;font-size:12px;text-align:right;}"
    ...
    if df.empty:
        ...
        bucket_order = ["current_week", "next_week", "monthly"]
        strategy_order = ["PUT", "CALL"]
        label_map = {
            "current_week": "Current Week",
            "next_week": "Next Week",
            "monthly": "Monthly",
            "PUT": "Top SELL PUT candidates",
            "CALL": "Top Covered CALL candidates",
        }
        ...
        for ticker in sorted(df["ticker"].unique()):
            ...
            for bucket in bucket_order:
                bdf = tdf[tdf["bucket"] == bucket]
                if bdf.empty:
                    continue
                bucket_label = bdf["bucket_label"].iloc[0]
                html_parts.append(f"<h3>{bucket_label} ({label_map[bucket]})</h3>")

                for strat in strategy_order:
                    sdf = bdf[bdf["strategy"] == strat].copy()
                    if sdf.empty:
                        continue
                    html_parts.append(f"<h4>{label_map[strat]}</h4>")
                    view = sdf[
                        [
                            "expiration",
                            "strike",
