...
recommendation rows for reporting.
"""
...
def _opt_float(cfg: Dict[str, Any], key: str, default: Optional[float]) -> Optional[float]:
...
def compute_ivr_proxy(price_df: pd.DataFrame, current_iv: Optional[float]) -> Tuple[Optional[float], str]:
    """
    IV Rank proxy using a rolling 20-day annualized HV series.

    IVR = (current_val - hv_low) / (hv_high - hv_low) * 100
    """
...
def get_support_levels(price_df: pd.DataFrame) -> Dict[str, Optional[float]]:
    ...
    low_col = price_df["Low"].astype(float) if "Low" in price_df.columns else price_df["Close"].astype(float)
    low_52w = float(low_col.min())
    swing_low_20d = float(low_col.rolling(20).min().iloc[-1]) if len(low_col) >= 20 else None
...
def recommend_csp_for_ticker(
    ...
    ivr_min = _opt_float(rec_config, "ivr_min", 30.0)
    earnings_buffer = int(rec_config.get("earnings_buffer_days", 7))
    delta_min = _opt_float(rec_config, "delta_min", 0.10)
    delta_max = _opt_float(rec_config, "delta_max", 0.25)
    ...
    monthly_puts = [c for c in candidates if c.get("strategy") == "PUT" and c.get("bucket") == "monthly"]
    if not monthly_puts:
        base["reason"] = "No monthly PUT candidates survived initial screening"
        return base

    current_iv = next((float(c["implied_volatility"]) for c in monthly_puts if c.get("implied_volatility")), None)
    ...
    def _delta_ok(c: Dict[str, Any]) -> bool:
        d = c.get("delta")
        if d is None:
            return False
        abs_delta = abs(float(d))
    ...
    if not qualified:
        reasons: List[str] = []
        if ivr_min is not None and ivr_value is not None and ivr_value < ivr_min:
        ...
        if delta_min is not None and delta_max is not None:
            ...
            reasons.append(f"no strike with |delta| >= {delta_min:.2f}")
        elif delta_max is not None:
            reasons.append(f"no strike with |delta| <= {delta_max:.2f}")
        else:
            reasons.append("no strike qualified")
    ...
    expiration_raw = best.get("expiration")
    if isinstance(expiration_raw, str):
        try:
            selected_expiration = date.fromisoformat(expiration_raw)
        except ValueError:
            selected_expiration = None
    elif isinstance(expiration_raw, date):
        selected_expiration = expiration_raw

    earnings_too_close = False
    if earnings_date is not None and selected_expiration is not None:
        days_before_expiry = (selected_expiration - earnings_date).days
        earnings_too_close = 0 <= days_before_expiry <= earnings_buffer
...
def build_csp_recommendations(
    ...
    """
    rec_config = {**DEFAULT_REC_CONFIG, **(config.get("csp_recommendation") or {})}
    ...
    results.sort(key=lambda r: (order.get(r["recommend"], 3), -(float(r.get("annualized_yield") or 0))))
